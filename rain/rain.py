...
class DateRainfall(object):
    def __init__(self, year, month, day, inches):
        self.year = year
        self.month = month
        self.day = day
        self.inches = inches
    ...
    def __repr__(self):
        return 'DateRainfall({!r}-{!r}-{!r}, {!r})'.format(
            self.year,
            self.month,
            self.day,
            self.inches
...
def read_file_to_get_date_and_rainfall(file_name):
    """Read the file to DateRainfall objects with date and rainfall amount

    :param file_name:
    :return:
    >>> read_file_to_get_date_and_rainfall(rain.txt)
    None
    """
    with open(file_name) as rain_file:
        for line in rain_file.readlines():
            ...
            if info_match != None:
                ...
                date_tuple = convert_date_to_std_format(rain_info[0])
                # add reformatted date back into the string
                # TAKE TUPLE AND PULL OUT YEAR, MONTH, DAY FOR OBJECT
                year = date_tuple[0]
                month = date_tuple[1]
                day = date_tuple[2]
                # rain_info[0] = date_string
                if len(rain_info) > 1:
                    # changes the number from '1' to the .01 format
                    rain_in_inches = float(rain_info[1]) * .01
                # create a DateRainfall object
                date_rainfall = DateRainfall(year, month, day, rain_in_inches)
                print(date_rainfall)
...
def convert_date_to_std_format(date_string):
    ...
    :param date_string:
    :return: tuple in (YYYY, M, DD) format
    """
    ...
    # year_month_day = (YYYY, M, DD)
    year_month_day = (year, month_num, day)
